...
1. **Server-only keys** — OpenRouter, Postiz, Fal, and future search/provider keys live in server environment variables or protected storage.
...
Schedule action resolves credentials via `resolvePostizCredentials()` and never returns keys to the client.
...
- Postiz responses are stored on `scheduled_posts.postiz_response` without echoing API keys.
