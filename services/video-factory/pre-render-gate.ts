...
import type { AudioMode, ProductionFormat } from "./production-options";
...
export function runPreRenderGate(input: PreRenderGateInput): PreRenderGateResult {
  ...
  if (!checks.duration_within_15pct) {
    const msg = `Storyboard duration ${totalDuration.toFixed(1)}s is ${(durationDelta * 100).toFixed(0)}% off target ${target}s`;
    if (audioModeUsesVoiceover(input.audioMode)) {
